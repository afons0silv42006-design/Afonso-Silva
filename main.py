...
echo -e "Memória RAM: ${VERDE}${MEM_USADA}MB / ${MEM_TOTAL}MB (${MEM_PERC}%)${RESET}"
...
if [ "$MEM_PERC" -gt 80 ]; then
    echo -e "${VERMELHO} ALERTA: Uso de Memória RAM acima de 80%${RESET}"
fi

if [ "$DISCO_PERC" -gt 90 ]; then
    echo -e "${VERMELHO} ALERTA: Pouco espaço em disco disponível${RESET}"
fi
